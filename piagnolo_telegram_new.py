            ...
            if next_req is not None:
                ...
                if next_req.name == "home/sala/stufa":
                    if float(next_req.args[0]) > 15 and float(next_req.args[0]) < 24:
                        ...
                        publish.single("home/sala/rele1", "1", hostname=hostname, port=1883)
                    else:
                        self.actual_setpoint = self.default
                        self.heater_enabled = False
...
class TelegramBarsanti:
    ...
    def keyboard(self, bot, update):
        custom_keyboard = [['Sala on', 'Sala off','Reset sala'], ['Camera on', 'Camera off', 'Reset camera'], ['Temperatura sala', 'Grafico sala']]
        reply_markup = telegram.ReplyKeyboardMarkup(custom_keyboard)
        bot.send_message(chat_id=update.message.chat.id, text="Seleziona il comando", reply_markup=reply_markup)

    def start(self, bot, update):
        update.message.reply_text('Benvenuto al centro di controllo piagnolo')
